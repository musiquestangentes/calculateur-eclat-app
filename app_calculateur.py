...
st.write("""
Cet outil vous permet de comprendre les éléments de votre fiche de paie
et de calculer vos heures et primes.
""")
...
if module == "Coefficient et salaire de base":
    ...
    st.sidebar.radio("Choisir un module :", ["Coefficient et salaire de base", "Heures lissées", "Primes", "Simulateur complet"])

    url_grille = "https://www.legifrance.gouv.fr/conv_coll/article/KALIARTI000048471347#KALIARTI000048471347"
    st.info(
        "**Coefficient :** Renvoie à la grille de classification de la convention collective ECLAT 1518, "
        "composée de groupes nommés par des lettres correspondant chacune à un coefficient et à une catégorie socioprofessionnelle.\n\n"
        "Les professeur·e·s sont rattaché·e·s par défaut au groupe A de niveau 1 (Ouvriers et employés), "
        "qui correspond au coefficient 247. Musiques Tangentes rattache ses profs au **groupe D** (Techniciens, agents de maîtrise), **coefficient 300**, "
        "dont le salaire de base est plus élevé."
    )
    st.markdown(
        "Nb : Le coefficient conventionnel de base, indiqué sur les bulletins de paie, est de 300 mais le coefficient réel "
        "sur lequel est indexé les paies des profs de Musiques Tangentes est de 362,03 (voir « prime différentielle » dans l'onglet \"Primes\"). "
        "Il est donc plus élevé que le coefficient maximal de la catégorie Techniciens et agents de maîtrise et s'approche de la catégorie Cadres."
    )
...
# PAGE 2 : HEURES LISSEES
    ...
    heures_annuelles = st.number_input(
        "Heures annuelles réellement effectuées (de septembre à août):",
        min_value=0.0,
        step=0.5
    )
    ...
    if heures_annuelles > 0:
        ...
        st.markdown("*Formule :*")
        st.latex("\\text{Heures mensuelles ETP} = \\frac{(\\text{Heures hebdo lissées} \\times 151,67)}{24}")
...
# PAGE 3 : PRIMES
    ...
    st.markdown(f"[>> Lien Légifrance - Valeur du point d'indice]({url_valpoint})")
    ...
    if heures_lissees > 0:
        ...
        st.info("La prime différentielle assure que tou·te·s les salarié·e·s soient sur un pied d'égalité en termes de rémunération, quelle que soit leur ancienneté.")
        st.markdown("*Formules :*")
